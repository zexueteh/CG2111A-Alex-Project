#include <serialize.h>
#include <stdarg.h> // for debugging
#include <math.h>
#include "packet.h"
#include "constants.h"
...
// and substitue with the correct values.
#define ALEX_LENGTH 17 // TO BE UPDATED
#define ALEX_BREADTH 10 // TO BE UPDATED
...
#define COUNTS_PER_REV      16 // TO BE UPDATED

// Wheel circumference in cm.
// We will use this to calculate forward/backward distance traveled
// by taking revs * WHEEL_CIRC

#define WHEEL_CIRC          21.362812 // TO BE UPDATED
...
unsigned long computeDeltaTicks(float ang) {
  ...
  unsinged long ticks = (unsigned long) ((ang * alexCirc * COUNTS_PER_REV) / (360 * WHEEL_CIRC));
...
void left(float ang, float speed) {
  ...
  ccw(ang, speed); // might need to change if we want to add PWM
}
...
void right(float ang, float speed) {
  ...
  cw(ang, speed); // might need to change if we want to add PWM
}
...
void setupEINT()
{
  ...
  EIMSK = 0b00001100; // Enable interrupts for INT3 and INT2 by setting bit 3 and bit 4
  EICRA = 0b10100000; // External interrupts 3 and 2 are configured on falling edge
